...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = X + np.random.normal(mu, np.sqrt(sigma2), N)  # Generate Y with normal additive error
    ...
    for _ in range(S):
        ...
        Y_sim = X_sim + np.random.normal(mu, np.sqrt(sigma2), N)  # Generate Y values with normal error
